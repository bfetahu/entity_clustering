...
import lsh.LocalitySentitiveHashing;
...
/**
 * Perform entity bucketing based on the LSH algorithm. Be careful, the process uses all CPU cores, however, this can be
 * changed from a parallel to a sequential system.
 *
 * Created by besnik on 11/23/14.
 */
public class LSHClustering {
    private final static Logger log = Logger.getLogger(LSHClustering.class.getName());

    public static void main(String[] args) {
        String out_dir = "", data_file = "";
    ...
    private void performSampleLSH(LocalitySentitiveHashing lsh, Set<Integer> entity_indices,
                                  THashMap<Integer, THashMap<Integer, TIntIntHashMap>> entity_text,
                                  THashMap<Integer, String> text_terms, String type) {
        ...
        log.info(String.format("Finished computing LSH clusters for type %s.\n", type));
    }
}
