...
public class EntityClustering {
    private final static Logger log = Logger.getLogger(EntityClustering.class.getName());

    public static void main(String[] args) throws Exception {
        String operation = "", btc_data = "", out_dir = "",
                data_file = "", lsh_bins = "", stop_words_path = "",
                type_filter = "", type_index_path = "",
                loaded_type_index = "", feature_data = "";
        ...
        //generate the type index where every entity is associated with at least on of the types it belongs.
        //NOTE: We assign to only one entity type to avoid duplicate cluster computations.
        if (operation.equals("type_index")) {
            Set<String> type_filter_set = FileUtils.readIntoSet(type_filter, "\n", false);
            ec.generateTypeIndex(type_filter_set, btc_data, out_dir);
        }
        //from the generated entity-type index generate separate files with the entity profiles.
        //NOTE: This is used later on to compute the entity feature vectors.
        else if (operation.equals("entity_type_index")) {
            //the entity type index. <entity_URI, type_id>
            THashMap<String, Integer> type_index = new THashMap<>();
            ec.loadTypeIndex(type_index, type_index_path);
            log.info(String.format("Loading entity-type index with %s items.\n", type_index.size()));

            //iterate over the BTC dataset and put into separate files the entity profiles belonging to the same type.
            ec.loadEntitiesForGraphAndTypes(type_index, btc_data, out_dir);
        }
        //after generating the type index and the specific entity profiles belonging to a specifc entity type
        //generate the entity feature vectors which are stored again in files for later use in the clustering process.
        //Finally, here for every entity we extract the label, body (consisting of all textual literals) and the set of entity linking statements.
            ...
            //we avoid here specific well known predicates that are common across any dataset/entity and
            //hence wouldn't give us any important information in the entity clustering process
        ...
        //generate the actual feature data structures that are later used for the specific types.
        //NOTE: here for every entity type all entity feature vectors are computed and stored into the corresponding datastructures
        //this is later used to finally generate the final feature vector.
        ...
        //cluster features: From the pre-computed cluster features for the different entity types, now compute the cluster feature vectors
        //this is the final feature files for an entity in the form of sparse 2D tuples.
        //The 2D feature space consists of all the textual literals and object literals already existing in all entities. Since the feature
        //vectors are sparse vectors we only place the <feature_indice:feature_value> tuples for only those feature items that are present in a specific entity
        ...
        //perform the x-means clustering on the generated entity feature vectors
        //NOTE: Here we need to set the desired maximum number of clusters.
        else if (operation.equals("xmeans")) {
            ec.performCOXMeans(out_dir, data_file, k, feature_data);
        }
        //perform the Spectral clustering on the generated entity feature vectors
        //NOTE: Here we need to set the desired maximum number of clusters.
    ...
    private void generateTypeInstanceFeatures(Set<String> type_files, String out_dir, String data_file_f, int ngram_f, Set<String> stop_words) {
        type_files.parallelStream().forEach(type_file -> {
            ...
            log.info(String.format("Finished processing type: %d.\n", type_id));
        });
    }
    ...
    private void buildClusterProfiles(Set<String> type_files, String type_index_path_f, String loaded_type_index_f, String data_file_f, String out_dir_f, Set<String> stop_words_set, Set<String> filter_predicates) {
        type_files.parallelStream().forEach(type_file -> {
            try {
                ...
                log.info(String.format("Finished computing features for entities of type: %d.\n", type_id));
            } catch (Exception e) {
                e.printStackTrace();
                log.severe(String.format("Error processing type: %s with message: %s.\n", type_file, e.getMessage()));
            }
        });
    }
    ...
    private void performSpectralClustering(String out_dir_f, String data_file_f, int finalK) {
        ...
        files.parallelStream().forEach(file -> {
            ...
            IntStream.range(0, tpl_splits.size()).parallel().forEach(split -> {
                ...
                IntStream.range(0, tpl.length).forEach(i -> {
                    ...
                    IntStream.range(i, tpl.length).forEach(j -> {
                        ...
                        m.set(i, j, MetricUtils.getEucledianDistance(vector_a, vector_b));
                    });
                });

                log.info(String.format("Finished loading the symmetric matrix for file %s and split %d.\n", file, split));
            ...
            log.info(String.format("Finished computing spectral clusters for bin %s and type %s.\n", lsh_bin, type));
        });
    }
            ...
            try {
                ...
                try {
                    ...
                    if (entity_indices == null || entity_indices.isEmpty()) {
                        log.info(String.format("There is no data about %s\n", data_dir + "/" + type + "/" + lsh_bin + "_indices.csv.gz"));
                        return;
                    }
                    StringBuffer sb = new StringBuffer();
                    for (int cluster_id : clusters.keySet()) {
                    ...
                    FileUtils.saveText(sb.toString(), out_file, false, true);
                    log.info(String.format("Finished computing x-means for bin %s and type %s.\n", lsh_bin, type));
    ...
    private void filterFeatureObjects(THashMap<Integer, TIntIntHashMap> text_counts,
        ...
        if (text_counts != null) {
            for (int ngram : text_counts.keySet()) {
                TIntIntIterator term_iterator = text_counts.get(ngram).iterator();
                while (term_iterator.hasNext()) {
                    int count = text_counts.get(ngram).get(term_iterator.key());
            ...
            if (object_counts.size() != 0) {
                TIntIntIterator object_iterator = object_counts.iterator();
                while (object_iterator.hasNext()) {
                    int count = object_counts.get(object_iterator.key());
                    if (total_entities < 100000) {
                        if (count < 2) {
    ...
    private void computeClusteringTupleFeatures(THashMap<Integer, TIntHashSet> lsh_bins,
        ...
        lsh_bins.keySet().parallelStream().forEach(bin_entry -> {
            try {
                ...
                log.info(String.format("%d entities in the LSH bin %s and for type %s, with %d objects, %d text-terms and %d label-terms.\n", total_entities, bin_name, type, entity_object_indices.size(), text_terms_total, label_terms_total));
    ...
    private void computeTupleFeatures(TIntHashSet entity_indices,
        ...
        log.info(String.format("Finished creating tuples list with %d attributes and %d tuples for type %s.\n", total_attributes, tuple_no, type));
        FileUtils.saveText(sb.toString(), out_dir + "/" + type + "/" + lsh_bin + "_tuples.obj", true, false);
    ...
    private void filterZeroEntities(TIntHashSet entity_indices, int[] object_indices,
        ...
        log.info(String.format("After removing entities with zero features %d/%d of entities.\n", total, entity_indices.size()));
    }
            ...
            try {
                ...
                while (reader.ready()) {
                    ...
                    try {
                        ...
                        log.severe(String.format("Skipping triple: %s with error %s\n", line, e.getMessage()));
                    }
                }
                FileUtils.saveText(sb.toString(), out_dir + "/type_index.csv", true, false);
                log.info(String.format("Finished processing file %s.\n", btc_file));
            } catch (Exception e) {
                log.severe(String.format("Skipping file: %s.\n", btc_file));
            }
        });
        StringBuffer sb = new StringBuffer();
    ...
    public void loadEntitiesForGraphAndTypes(THashMap<String, Integer> type_index,
        ...
        btc_files.parallelStream().forEach(btc_file -> {
            try {
                ...
                log.info(String.format("Finished processing file: \t" + btc_file));
            } catch (Exception e) {
                log.info(String.format("Error parsing file: %s with error message: %s\n", btc_file, e.getMessage()));
            }
        });
    }
    ...
    public List<TupleList> loadSparseTuples(String file) {
        try {
            ...
            log.info(String.format("Loading tuples for file %s which is split into %d splits from %d entities\n", file, splits, entity_no));
    ...
    public List<int[][][]> optimizedLoadTuple2DArray(String file) {
        try {
            ...
            log.info(String.format("Loading tuples for file %s which is split into %d splits\n", file, splits));
